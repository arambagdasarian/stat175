...
def load_dataset(data_name):
    if data_name == "amlworld":
        # AMLWorld (HI-Small) graph saved by this project's training script.
        path = os.environ.get("AMLWORLD_DGL_GRAPH")
        if not path or not os.path.isfile(path):
            raise RuntimeError(
                "Set AMLWORLD_DGL_GRAPH to a file produced by "
                "scripts/graphmaker_train_amlworld.py (dgl.save_graphs)."
            )
...
def preprocess(g):
    ...
    for f in range(X.size(1)):
        # (N, 2) — always 2 classes (binary node attrs); infer-only one_hot breaks if a column is constant
        X_f_one_hot = F.one_hot(X[:, f].long(), num_classes=2)
    ...
    E = torch.zeros(N, N)
    E[dst, src] = 1.
    # (N, N, 2) — always absent/present; infer-only one_hot breaks when E is all zeros (empty graph)
    ...
    # (N, C) — at least 2 classes so one_hot shape is stable if only one label appears in a subgraph
    num_y = max(2, int(Y.max().item()) + 1)
    Y_one_hot = F.one_hot(Y.long(), num_classes=num_y).float()
    # (C)
